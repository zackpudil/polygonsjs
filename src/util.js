...
export function createShadowMap(gl, width, height) {
  ...
  var colorTexture = new Texture(createTexture(gl, [width, height], gl.RGBA), 'blank');

  var depthBuffer = gl.createFramebuffer();
  gl.bindFramebuffer(gl.FRAMEBUFFER, depthBuffer);
  gl.framebufferTexture2D(gl.FRAMEBUFFER, gl.COLOR_ATTACHMENT0, gl.TEXTURE_2D, colorTexture.texture.handle, 0);
  gl.framebufferTexture2D(gl.FRAMEBUFFER, gl.DEPTH_ATTACHMENT, gl.TEXTURE_2D, depthMap.texture.handle, 0);
